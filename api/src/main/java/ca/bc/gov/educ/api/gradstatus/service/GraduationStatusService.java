...
//            }
    ...
    @Transactional
    public Pair<GraduationStatus, GradStatusEvent> ungradStudent(UUID studentID, String ungradReasonCode, String accessToken) throws JsonProcessingException {
        if(StringUtils.isNotBlank(ungradReasonCode)) {
    		...
    		if(ungradReasonObj != null) {
		        ...
		        if (gradStatusOptional.isPresent()) {
		            GraduationStatusEntity gradEntity = gradStatusOptional.get();
		            saveUngradReason(gradEntity.getPen(),studentID,ungradReasonCode,accessToken);
                    gradEntity.setRecalculateGradStatus("Y");
